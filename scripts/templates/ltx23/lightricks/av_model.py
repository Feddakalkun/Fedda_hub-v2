...
class GatedCrossAttention(CrossAttention):
    def __init__(
        ...
        **kwargs,
        ...
        if gated:
            self.to_gate_logits = operations.Linear(
                query_dim, heads, bias=True, dtype=dtype, device=device
            )
    ...
    def forward(self, x, context=None, mask=None, pe=None, k_pe=None, transformer_options={}):
        ...
        if pe is not None:
            q = self._apply_rotary_emb(q, pe)
            k = self._apply_rotary_emb(k, pe if k_pe is None else k_pe)
        ...
        if mask is None:
            ...
            out = comfy.ldm.modules.attention.optimized_attention_masked(
                q, k, v, self.heads, mask, attn_precision=self.attn_precision, transformer_options=transformer_options
            )
    ...
    @staticmethod
    def _apply_rotary_emb(input_tensor, freqs_cis):
        from comfy.ldm.lightricks.model import apply_rotary_emb

        return apply_rotary_emb(input_tensor, freqs_cis)
...
class BasicAVTransformerBlock(nn.Module):
    def __init__(
        ...
        self.audio_attn1 = GatedCrossAttention(
            query_dim=a_dim,
            heads=a_heads,
            dim_head=ad_head,
            context_dim=None,
        ...
        self.attn2 = GatedCrossAttention(
            query_dim=v_dim,
        ...
        self.audio_attn2 = GatedCrossAttention(
            query_dim=a_dim,
        ...
        # Q: Video, K,V: Audio
        self.audio_to_video_attn = GatedCrossAttention(
            query_dim=v_dim,
            context_dim=a_dim,
            heads=a_heads,
            dim_head=ad_head,
        ...
        # Q: Audio, K,V: Video
        self.video_to_audio_attn = GatedCrossAttention(
            query_dim=a_dim,
            context_dim=v_dim,
            heads=a_heads,
            dim_head=ad_head,
        ...
        ada_rows = 9 if self.cross_attention_adaln else 6
        self.scale_shift_table = nn.Parameter(torch.empty(ada_rows, v_dim, device=device, dtype=dtype))
        self.audio_scale_shift_table = nn.Parameter(
            torch.empty(ada_rows, a_dim, device=device, dtype=dtype)
        )
        if self.cross_attention_adaln:
            self.prompt_scale_shift_table = nn.Parameter(
                torch.empty(2, v_context_dim, device=device, dtype=dtype)
            )
            self.audio_prompt_scale_shift_table = nn.Parameter(
                torch.empty(2, a_context_dim, device=device, dtype=dtype)
            )
    ...
    def _apply_text_cross_attention(
        ...
        if self.cross_attention_adaln:
            ...
            attn_input = comfy.ldm.common_dit.rms_norm(x) * (1 + scale_q) + shift_q
            if prompt_scale_shift_table is not None and prompt_timestep is not None:
                ...
                context = context * (1 + scale_kv) + shift_kv
            out = attn(attn_input, context=context, mask=attention_mask, transformer_options=transformer_options)
            return out * gate
        return attn(
    ...
    def forward(
        ...
        if run_vx:
            ...
            vx.add_(self._apply_text_cross_attention(
                ...
                v_timestep,
                transformer_options.get("v_prompt_timestep"),
                attention_mask,
                transformer_options,
            ))
        ...
        if run_ax:
            ...
            ax.add_(self._apply_text_cross_attention(
                ...
                a_timestep,
                transformer_options.get("a_prompt_timestep"),
                attention_mask,
                transformer_options,
            ))
...
class LTXAVModel(LTXVModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.caption_projection_first_linear = kwargs.get("caption_projection_first_linear", True)
        self.caption_projection_second_linear = kwargs.get("caption_projection_second_linear", True)
        self.caption_proj_before_connector = kwargs.get("caption_proj_before_connector", False)
        self.caption_proj_input_norm = kwargs.get("caption_proj_input_norm", True)
    ...
    def _init_model_components(self, device, dtype, **kwargs):
        ...
        super()._init_model_components(device, dtype, **kwargs)
        main_ada_coeff = 9 if kwargs.get("cross_attention_adaln", False) else 6
        if not self.caption_projection_first_linear and not self.caption_projection_second_linear:
            self.caption_projection = None
        self.adaln_single = AdaLayerNormSingle(
            self.inner_dim,
            embedding_coefficient=main_ada_coeff,
        ...
        if kwargs.get("cross_attention_adaln", False):
        ...
        self.audio_embeddings_connector = Embeddings1DConnector(
            in_channels=self.audio_cross_attention_dim,
            attention_head_dim=kwargs.get("audio_connector_attention_head_dim", self.audio_attention_head_dim),
            num_attention_heads=kwargs.get("audio_connector_num_attention_heads", self.audio_num_attention_heads),
        ...
        self.video_embeddings_connector = Embeddings1DConnector(
            in_channels=self.cross_attention_dim,
            attention_head_dim=kwargs.get("connector_attention_head_dim", self.attention_head_dim),
            num_attention_heads=kwargs.get("connector_num_attention_heads", self.num_attention_heads),
            num_layers=kwargs.get("connector_num_layers", 2),
            positional_embedding_max_pos=kwargs.get("connector_positional_embedding_max_pos", [4096]),
            num_learnable_registers=kwargs.get("connector_num_learnable_registers", 128),
            gated_attention=self.connector_apply_gated_attention,
    ...
    def preprocess_text_embeds(self, context):
        already_split_dim = self.cross_attention_dim + self.audio_cross_attention_dim
        if context.shape[-1] == already_split_dim or context.shape[-1] == self.caption_channels * 2:
            return context
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                BasicAVTransformerBlock(
                    ...
                    cross_attention_adaln=kwargs.get("cross_attention_adaln", False),
                    gated_attention=self.connector_apply_gated_attention,
                    dtype=dtype,
                    device=device,
                    operations=self.operations,
    ...
    def _prepare_timestep(self, timestep, batch_size, hidden_dtype, **kwargs):
        ...
        if self.prompt_adaln_single is not None:
            v_prompt_timestep, _ = self.prompt_adaln_single(
                timestep_scaled.flatten(),
                {"resolution": None, "aspect_ratio": None},
                batch_size=batch_size,
                hidden_dtype=hidden_dtype,
            )
            prompt_timesteps[0] = v_prompt_timestep.view(batch_size, -1, v_prompt_timestep.shape[-1])
        if self.audio_prompt_adaln_single is not None:
            audio_prompt_source = a_timestep_scaled.flatten() if kwargs.get("a_timestep") is not None else timestep_scaled.flatten()
            a_prompt_timestep, _ = self.audio_prompt_adaln_single(
                audio_prompt_source,
                {"resolution": None, "aspect_ratio": None},
                batch_size=batch_size,
                hidden_dtype=hidden_dtype,
            )
            prompt_timesteps[1] = a_prompt_timestep.view(batch_size, -1, a_prompt_timestep.shape[-1])

        return [v_timestep, a_timestep, cross_av_timestep_ss, prompt_timesteps], [
            v_embedded_timestep,
            a_embedded_timestep,
        ]
    ...
    def _prepare_context(self, context, batch_size, x, attention_mask=None):
        ...
        if context.shape[-1] == self.cross_attention_dim + self.audio_cross_attention_dim:
            v_context, a_context = torch.split(
                context,
                [self.cross_attention_dim, self.audio_cross_attention_dim],
                len(context.shape) - 1,
            )
        else:
            v_context = context
        ...
        if self.caption_projection is not None:
            ...
            v_context = v_context.view(batch_size, -1, vx.shape[-1])
    ...
    def _process_transformer_blocks(
        ...
        if len(timestep) > 3:
            v_prompt_timestep, a_prompt_timestep = timestep[3]
